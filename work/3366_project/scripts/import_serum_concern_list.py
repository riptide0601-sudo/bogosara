...
def _strip_label_prefix(text: str) -> str:
    """"[피디알엔 세럼] 정제수, ..." / "PDRN 앰플 30ml: 정제수,..." 같은 라벨 접두를 제거."""
    ...
    colon_idx = text.find(":")
    comma_idx = text.find(",")
    if colon_idx != -1 and colon_idx < 40 and (comma_idx == -1 or colon_idx < comma_idx):
        return text[colon_idx + 1 :]
    return text


def _split_ingredients(text: str) -> list[str]:
    """쉼표로 분리하되, 괄호 안의 쉼표(예: "(30,000 ppm)")는 무시한다."""
    tokens: list[str] = []
    current: list[str] = []
    depth = 0
    for ch in text:
...
def parse_ingredient_tokens(raw_text: str) -> list[str]:
    """전성분 원문을 (라벨 접두·각주 제거된) 성분 토큰 리스트로 분리합니다."""
    text = _strip_label_prefix((raw_text or "").strip())
    tokens = _split_ingredients(text)
    # 뒤에 " * 설명: ..." 형태 각주가 마지막 토큰에 붙어 나오는 경우가 있어 제거.
    return [t.split("*")[0].strip() for t in tokens if t.split("*")[0].strip()]
...
def _find_or_create_product(db: Session, product_name: str) -> tuple[Product, bool]:
    product = db.scalars(
        select(Product).where(Product.product_name == product_name)
    ).first()
    if product is not None:
        return product, False
    product = Product(product_name=product_name, category=classify_category(product_name).name)
    db.add(product)
    db.flush()
    return product, True


def _upsert_product_ingredient(db: Session, product_id: str, ingredient_id: int, label_rank: int, matched_text: str) -> None:
    ...
    with Session(engine) as db:
        for sheet_name in wb.sheetnames:
            ...
            for _, product_name, raw_ingredients in rows:
                ...
                product, created = _find_or_create_product(db, product_name)
                ...
                for rank, token in enumerate(tokens, start=1):
                    total_tokens += 1
                    clean = _clean_for_matching(token)
                    ...
                    if not dry_run:
                        ...
                        for ingredient_id in ids:
                            _upsert_product_ingredient(db, product.product_id, ingredient_id, rank, token)
